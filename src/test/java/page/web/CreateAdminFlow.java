...
public class CreateAdminFlow extends BasePage {
    ...
    @Step ("Creating New KPI Definition")
    public  void createNewKPIDefinition(String name) throws InterruptedException {
        try {
            ...
            System.out.println("Unable to Create New KPI");
        }
    }
    ...
    public void addLogicToTheKPIAndValidate(String name,String text,String SearchName) throws InterruptedException {
        try {
            ...
            System.out.println("Unable to Validate API");
        }
    }
        private void editGlobalParameterImplementations(String name , String text) {
            ...
            Locator singeingSection = childPage.locator("div").filter(new Locator.FilterOptions().setHas(equipmentHeader)).last();
         ...
         private  void editKPIImplementation(String SearchName) {
            ...
            expandBtn.click();
            Locator machineHeader = childPage.locator("h4").filter(new Locator.FilterOptions().setHas(childPage.locator("span.machine-name", new Page.LocatorOptions().setHasText("SINGEING"))));
            machineHeader.scrollIntoViewIfNeeded();
            machineHeader.click();
            Locator singeingSectionn = childPage.locator("div").filter(new Locator.FilterOptions().setHas(machineHeader)).last();
